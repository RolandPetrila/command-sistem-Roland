...
Invoice CRUD sub-router:
  ...
  - Overdue invoices
  - Item presets
  - Invoice from ITP / from calculation
  - e-Factura XML export
  - ANAF CUI verification
  - Send invoice by email
  - Currency conversion (BNR rates)

Recurring invoices and partial payments are in separate files
(recurring.py, payments.py) and included via include_router().
"""
...
# Import sub-routers for recurring and payments
from .recurring import recurring_router
...
# Mount sub-routers so all endpoints are accessible via crud_router
crud_router.include_router(recurring_router)
crud_router.include_router(payments_router)
...
async def create_client(data: ClientCreate):
    ...
    async with get_db() as db:
        cursor = await db.execute(
            """INSERT INTO clients (name, cui, reg_com, address, email, phone, notes, default_payment_terms)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (data.name, data.cui, data.reg_com, data.address, data.email, data.phone, data.notes, data.default_payment_terms),
        )
        await db.commit()
...
async def create_invoice(data: InvoiceCreate):
    ...
    # R4-27: Duplicate detection — check same client + same day
    warning = None
    duplicate_id = None
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id, invoice_number FROM invoices WHERE client_id = ? AND date = ? ORDER BY id DESC LIMIT 1",
            (data.client_id, data.date),
        )
        dup_row = await cursor.fetchone()
        if dup_row:
            warning = f"Exista o factura similara (#{dup_row['invoice_number']}) din aceeasi zi pentru acest client"
            duplicate_id = dup_row["id"]
    ...
    # R4-29: Auto-calculate due_date from client payment terms if not provided
    effective_due_date = data.due_date
    if not effective_due_date:
        async with get_db() as db:
            cursor = await db.execute(
                "SELECT default_payment_terms FROM clients WHERE id = ?", (data.client_id,)
            )
            client_row = await cursor.fetchone()
            terms = client_row["default_payment_terms"] if client_row else None
            if terms:
                terms_days = {"immediate": 0, "net_15": 15, "net_30": 30, "net_45": 45}
                days = terms_days.get(terms)
                if days is not None:
                    try:
                        inv_date = date.fromisoformat(data.date)
                        effective_due_date = (inv_date + timedelta(days=days)).isoformat()
                    except ValueError:
                        pass
    ...
    async with get_db() as db:
        cursor = await db.execute(
            """INSERT INTO invoices
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft')""",
            (
                data.client_id, invoice_number, data.date, effective_due_date,
...
# ═══════════════════════════════════════════
# Currency conversion via BNR rates
# (MUST be registered BEFORE /{invoice_id} to avoid route conflict)
# ═══════════════════════════════════════════

_bnr_invoice_cache: dict = {"data": None, "ts": 0.0}
_BNR_URL = "https://www.bnr.ro/nbrfxrates.xml"
_BNR_NS = {"bnr": "http://www.bnr.ro/xsd"}
_BNR_CACHE_TTL = 3600  # 1 hour


async def _fetch_bnr_rates_for_invoice() -> dict:
    """Fetch BNR exchange rates with 1h cache. Returns {date, rates}."""
    now = time.time()
    if _bnr_invoice_cache["data"] and (now - _bnr_invoice_cache["ts"]) < _BNR_CACHE_TTL:
        return _bnr_invoice_cache["data"]
    ...
    try:
        ...
        logger.warning("BNR fetch error (invoice): %s", exc)
        if _bnr_invoice_cache["data"]:
            return _bnr_invoice_cache["data"]
        raise HTTPException(502, "Nu s-a putut accesa BNR si nu exista date in cache.") from exc
    ...
    result = {"date": rate_date, "rates": rates}
    _bnr_invoice_cache["data"] = result
    _bnr_invoice_cache["ts"] = now
    return result


@crud_router.get("/convert-price")
async def convert_invoice_price(
    amount: float = Query(..., description="Suma de convertit"),
    from_currency: str = Query("RON", alias="from", description="Moneda sursa (ex: EUR)"),
    to_currency: str = Query("RON", alias="to", description="Moneda destinatie (ex: EUR)"),
):
    """Conversie pret factura folosind cursul BNR curent.

    Returneaza suma originala, suma convertita, cursul si data cursului.
    Util pentru afisarea echivalentului in alta moneda pe facturi.
    """
    ...
    bnr_data = await _fetch_bnr_rates_for_invoice()
    rates = bnr_data["rates"]
    rate_date = bnr_data["date"]

    # Convert from source to RON first
    if fr == "RON":
        amount_ron = amount
    elif fr in rates:
        amount_ron = amount * rates[fr]
    else:
        raise HTTPException(400, f"Moneda sursa necunoscuta: {fr}. Disponibile: RON, {', '.join(sorted(rates.keys()))}")

    # Convert from RON to target
    if to == "RON":
        converted = amount_ron
    elif to in rates:
        converted = amount_ron / rates[to]
    else:
        raise HTTPException(400, f"Moneda destinatie necunoscuta: {to}. Disponibile: RON, {', '.join(sorted(rates.keys()))}")

    # Calculate the effective exchange rate between the two currencies
    if fr == "RON":
        effective_rate = 1.0 / rates[to]
    elif to == "RON":
        effective_rate = rates[fr]
    else:
        effective_rate = rates[fr] / rates[to]
...
@crud_router.get("/items/recent/{client_id}")
async def recent_items_for_client(client_id: int):
    """Ultimele 10 articole unice folosite in facturile unui client."""
    async with get_db() as db:
        cursor = await db.execute(
        ...
        cursor = await db.execute(
            """SELECT items_json FROM invoices
               WHERE client_id = ?
...
# ═══════════════════════════════════════════
# e-Factura XML export (UBL 2.1 / RO_CIUS)
# ═══════════════════════════════════════════

@crud_router.get("/{invoice_id}/efactura-xml")
async def export_efactura_xml(invoice_id: int):
    """Export factura in format e-Factura XML (UBL 2.1 / RO_CIUS) pentru ANAF."""
    from .efactura import generate_efactura_xml
    ...
    client = {
        ...
        "cui": invoice.get("client_cui", ""),
        "address": invoice.get("client_address", ""),
    }

    company = {
        "name": os.getenv("COMPANY_NAME", "CIP Inspection SRL"),
        "cui": os.getenv("COMPANY_CUI", "43978110"),
        "address": os.getenv("COMPANY_ADDRESS", ""),
        "iban": os.getenv("COMPANY_IBAN", ""),
    }

    xml_str = generate_efactura_xml(invoice, client, company, items)

    inv_number = invoice.get("invoice_number", f"INV-{invoice_id}")
    filename = f"efactura_{inv_number}.xml"

    await log_activity(
        action="invoice.efactura_export",
        summary=f"e-Factura XML exportata: {inv_number}",
        details={"invoice_id": invoice_id, "invoice_number": inv_number},
    )
...
class CommLogCreate(_BaseModel):
    comm_type: str = "note"  # note, email, phone, meeting
    summary: str
    details: str = ""


@crud_router.get("/clients/{client_id}/comm-log")
async def list_comm_log(client_id: int):
    """List communication entries for a client."""
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id FROM clients WHERE id = ?", (client_id,)
        )
        ...
        cursor = await db.execute(
            "SELECT id, client_id, comm_type, summary, details, date, created_at "
            "FROM client_comm_log WHERE client_id = ? ORDER BY created_at DESC",
            (client_id,),
        )
        rows = await cursor.fetchall()
...
@crud_router.post("/clients/{client_id}/comm-log", status_code=201)
async def create_comm_log(client_id: int, data: CommLogCreate):
    """Add a communication entry for a client."""
    if not data.summary.strip():
        raise HTTPException(400, "Sumar comunicare obligatoriu.")
    ...
    async with get_db() as db:
        ...
        cursor = await db.execute(
            ...
            "VALUES (?, ?, ?, ?, ?)",
            (client_id, data.comm_type, data.summary.strip(), data.details, today),
        )
        await db.commit()
        entry_id = cursor.lastrowid

    await log_activity(
        action="invoice.comm_log_create",
        summary=f"Comunicare client {client_row['name']}: {data.comm_type} — {data.summary[:50]}",
        details={"client_id": client_id, "entry_id": entry_id, "comm_type": data.comm_type},
    )
    return {"id": entry_id, "message": "Intrare comunicare adaugata cu succes."}


@crud_router.delete("/clients/{client_id}/comm-log/{entry_id}")
async def delete_comm_log(client_id: int, entry_id: int):
    """Delete a communication log entry."""
    async with get_db() as db:
        cursor = await db.execute(
            "SELECT id FROM client_comm_log WHERE id = ? AND client_id = ?",
            (entry_id, client_id),
        )
        if not await cursor.fetchone():
            raise HTTPException(404, "Intrare comunicare negasita.")

        await db.execute("DELETE FROM client_comm_log WHERE id = ?", (entry_id,))
        await db.commit()

    await log_activity(
        action="invoice.comm_log_delete",
        summary=f"Comunicare stearsa: client #{client_id}, intrare #{entry_id}",
        details={"client_id": client_id, "entry_id": entry_id},
    )
    return {"message": "Intrare comunicare stearsa cu succes."}
